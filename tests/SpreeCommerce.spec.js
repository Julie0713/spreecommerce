...
test("Verify user browse and add products to cart", async ({ page }) => {
  ...
  await page.waitForTimeout(3000);
  await expect(page.locator("#product-272")).toContainText(
    "Gold Bracelet Regular price $99.99"
  );
...
test("Verify user can view cart and validate name, quantity and price", async ({
  page,
}) => {
  const { email, password } = creds;
  await page
  ...
  await page
    .getByRole("listitem")
    .filter({ hasText: "Blue Denim Shirt $55.99 Blue M" })
    .click();
  await page.locator("$55.99");
  await page.locator("2");
  await page
    .getByRole("listitem")
    .filter({ hasText: "Gold Bracelet $99.99 Gold" })
    .click();
  await page.locator("$99.99");
  await page.locator("1");
});

test("Verify user can proceed to check out items in the cart", async ({
  page,
}) => {
  const { email, password } = creds;
  await page
...
test("Verify user can add a shipping address after checking out items", async ({
  page,
}) => {
  const { email, password } = creds;
  await page
  ...
  await page
    ...
    .pressSequentially("red h", { delay: 1200 });
  await page
    .getByRole("option", { name: "Red Hotel Cubao, Quezon City" })
    .click();
...
test("Verify user can select a shipping method", async ({ page }) => {
  ...
  await expect(page.locator("h5")).toContainText(
    "Delivery method from Shop location"
  );
...
test("Verify different delivery and pricing options", async ({ page }) => {
  ...
  await page.getByRole("link", { name: "Checkout" }).click();
  await expect(
    page.getByLabel("breadcrumb").getByText("Delivery")
  ).toBeVisible();

  await expect(
    page.getByText("Standard Delivery in 3-5 business days $")
  ).toBeVisible();
  await expect(
    page.getByText("Premium Delivery in 2-3 business days $")
  ).toBeVisible();
  await expect(
    page.getByText("Next Day Delivery in 1-2 business days $")
  ).toBeVisible();
  await page.getByRole("radio", { name: "Standard Delivery in 3-5" }).check();
  await page.getByRole("radio", { name: "Premium Delivery in 2-3" }).check();
  await page.getByRole("radio", { name: "Next Day Delivery in 1-2" }).check();
  ...
  await page.waitForTimeout(3000);
  await expect(
    page.getByLabel("breadcrumb").getByText("Payment")
  ).toBeVisible();
});

test("Verify different payment method and payment via card", async ({
  page,
}) => {
  const { email, password } = creds;
  await page
  ...
  const stripeFrameLocator = page.frameLocator(
    '#checkout_payment_methods iframe[title="Secure payment input frame"]:not([aria-hidden="true"])'
  );
  ...
  await expect(
    stripeFrameLocator.getByLabel("Additional Payment Methods")
  ).toBeVisible();
});
...
test("Verify order confirmation once payment is done", async ({ page }) => {
  ...
  // payment starts here
  await expect(page.locator("#checkout_payment_methods")).toContainText(
    "Payment"
  );
  await expect(page.locator("#checkout_payment_methods")).toContainText(
  ...
  await expect(
    stripeFrameLocator.getByRole("textbox", { name: "Card number" })
  ).toBeVisible();
  await stripeFrameLocator
    .getByRole("textbox", { name: "Card number" })
